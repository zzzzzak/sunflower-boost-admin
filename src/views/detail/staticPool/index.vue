...
<script lang="ts" setup>
  ...
  const [registerTable, { reload }] = useTable({
    ...
    api: ({ toTime, ...params }) => {
      ...
      return adminApi.staticPoolListPage(params);
    },
  ...
  async function handleSubmit({ editType, values }) {
    if (editType) {
      await adminApi.staticPoolUpdate(values);
      success('更新成功');
    } else {
      await adminApi.staticPoolCreate(values);
      success('创建成功');
    }
    reload();
    return true;
  ...
  async function handleDelete(record: Recordable) {
    await adminApi.staticPoolDelete(record);
    success('删除成功');
    reload();
  }
