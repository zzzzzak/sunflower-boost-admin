...
<script lang="ts" setup>
  ...
  const [registerTable, { reload }] = useTable({
    ...
    api: ({ toTime, ...params }) => {
      ...
      return adminApi.adminUserListPage(params);
    },
  ...
  async function handleSubmit({ editType, values }) {
    ...
    if (editType) {
      await adminApi.adminUserUpdate(values);
      success('更新成功');
    } else {
      await adminApi.adminUserCreate(values);
      success('创建成功');
    }
    reload();
    return true;
  ...
  async function handleDelete(record: Recordable) {
    await adminApi.adminUserDelete(record);
    success('删除成功');
    reload();
  }
</script>
