...
export function useColumns(
  ...
  const getViewColumns = computed(() => {
    ...
    const mapFn = (column) => {
      const { slots, customRender, format, edit, editRow, flag, valueEnum } = column;

      if (!slots || !slots?.title) {
        column.customTitle = column.title;
      }
      const isDefaultAction = [INDEX_COLUMN_FLAG, ACTION_COLUMN_FLAG].includes(flag!);
      if (!customRender && (format || valueEnum) && !edit && !isDefaultAction) {
        column.customRender = ({ text, record, index }) => {
          return formatCell(text, format, record, index, valueEnum);
        };
      }
...
export function formatCell(
  ...
  if (valueEnum) {
    const currItem = valueEnum!.find((item) =>
      isEqual(
        isNumber(item.value) ? String(item.value) : item.value,
        isNumber(text) ? String(text) : text,
      ),
    );
    if (!currItem) {
      return '';
    }
    if (currItem.customRender) {
      return currItem.customRender(currItem as any);
    }
    if (isUndefined(currItem.color)) {
      return currItem.label;
    }
    return h(Tag, { color: currItem.color }, () => currItem.label);
  }
  if (!format) {
    return text;
  }
