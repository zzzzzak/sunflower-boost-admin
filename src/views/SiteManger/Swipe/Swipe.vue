...
<script lang="ts" setup>
  ...
  import * as swipeApi from '@/api/sys/swipe';
  ...
  async function handleDelete(record: Recordable) {
    await swipeApi.deleteData(record);
  ...
  async function handleSubmit({ isUpdate, values }) {
    if (isUpdate) {
      await swipeApi.updateData(values);
      createSuccessModal({ title: '提示', content: '更新成功' });
    } else {
      await swipeApi.addData(values);
