      ...
      async () => {
        ...
        if (tableProps?.columns) {
          const columns = await Promise.all(
            unref(tableProps?.columns).map(async (item) => {
              if (item.asyncValueEnum && !item.valueEnum) {
                const valueEnum = await item.asyncValueEnum();
                return {
                  ...item,
                  valueEnum,
                };
          ...
          if (!isEqual(tableProps?.columns, columns)) {
